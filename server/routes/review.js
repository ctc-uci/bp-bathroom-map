...
router.post('/', async(req, res) => {
    try {
        ...
        const updated = await Bathroom.findOneAndUpdate({ _id: bathroomId }, //----> filter to choose bathroom on ID to update
        ...
        //send response
        res.status(200).json({ data: updated }); // sending status and response to the frontend
    } catch (err) {
        console.log(err);
        //error handling
